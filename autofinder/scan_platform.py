...
from autofinder.get_platform_z import get_platform_z_diff
#%%
class Scan_Platform():
    ...
    def start_scan_large(self, x_step = 2000, y_step = 2000, x_num = 25, y_num = 25):
    ...
    def start_scan_small(self, fast = False, scan_only = False, **kwargs):
        ...
        for i in range(num):
            ...
            z_diff = get_platform_z_diff(*self.stage.current_pos[:2], *points[0])
            self.stage.move_xyz(z = z_diff)
            self.stage.goto_xyz(*points[0])
            self.scanner.scan(points, subfolder = 'small_' + str(i), focus = True, mode = 'goto', fast = fast,
                              initial_focus_param=[10000, 10])
            if scan_only:
                continue
    ...
    def start_scan_revisit(self):
        ...
        for i in range(num):
            ...
            z_diff = get_platform_z_diff(*self.stage.current_pos[:2], *revisit_pos[0])
            self.stage.move_xyz(z = z_diff)
            self.stage.goto_xyz(*revisit_pos[0])
            self.scanner.scan(revisit_pos, subfolder = '20x_' + str(i), focus = True, mode = 'goto',
                              initial_focus_param = [10000, 10],focus_param = [4000, 10], focus_delay = 0.3,
...
if __name__ == '__main__':
    temp = Scan_Platform(Camera(0), Stage_Rpi())
    temp.initialize()
