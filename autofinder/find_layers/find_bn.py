...
#%%
def layer_search_bn(filename, background, area_thresh = 4000, thickness_range = [1, 50],
                    roi = [0.15, 0.75, 0, 1]):
    isLayer = False
    contrast_list = []
    flake_position_list = []
    ...
    # filename = 'F:/Temp/bn/small_0/2023-06-20--19-12-04-899733.jpg'
    ...
    hist_y = hist_y[50: 100]
    bk_color_y = np.argmax(hist_y) + 50
    
    # remove edge, maybe a better way to solve this is to find edge
    img_binary = cv2.inRange(img_gray, 0, 30)
    ...
    #%%
    threshold_lo_1 = [int(bk_color[0] * 1.05), int(bk_color[1] * 0.92), int(bk_color[2] * 0.7)]
    threshold_hi_1 = [int(bk_color[0] * 2.4), int(bk_color[1] * 2), int(bk_color[2] * 0.98)]
    img_binary_1 = cv2.inRange(img, np.array(threshold_lo_1), np.array(threshold_hi_1))

    threshold_lo_2 = [int(bk_color[0] * 1.5), int(bk_color[1] * 1.1), int(bk_color[2] * 0.7)]
    threshold_hi_2 = [int(bk_color[0] * 2.4), int(bk_color[1] * 3), int(bk_color[2] * 2.5)]
...
def test_run(background):
    filepath = 'F:/Temp/bn/small_0'
    pathDir =  os.listdir(filepath)
    # outpath = 'F:/Temp/gr/6-24/temp'
    resultpath = 'F:/Temp/bn/results_0_4000_test'
