...
#%%
def layer_search_bn(filename, background, area_thresh = 3000, thickness_range = [0.9, 10],
                    roi = [0, 1, 0, 1]):
    isLayer = False
    contrast_list = []
    flake_position_list = []
    ...
    # filename = 'F:/Temp/bn_new/0810-5/color_shift/2023-08-10--20-50-10-47B000.png'
    ...
    img_binary = cv2.inRange(img_gray, 0, 10)
    ...
    threshold_hi_1 = [int(bk_color[0] * 3.5), int(bk_color[1] * 3), int(bk_color[2] * 0.98)]
    img_binary_1 = cv2.inRange(img, np.array(threshold_lo_1), np.array(threshold_hi_1))

    threshold_lo_2 = [int(bk_color[0] * 2.7), int(bk_color[1] * 1.8), int(bk_color[2] * 0.8)]
    threshold_hi_2 = [int(bk_color[0] * 5.5), int(bk_color[1] * 7.5), int(bk_color[2] * 6)]
    img_binary_2 = cv2.inRange(img, np.array(threshold_lo_2), np.array(threshold_hi_2))

    threshold_lo_3 = [int(bk_color[0] * 0.8), int(bk_color[1] * 0.95), int(bk_color[2] * 2)]
    threshold_hi_3 = [int(bk_color[0] * 5), int(bk_color[1] * 7), int(bk_color[2] * 6)]
    img_binary_3 = cv2.inRange(img, np.array(threshold_lo_3), np.array(threshold_hi_3))

    img_binary = cv2.bitwise_or(img_binary_1, img_binary_2)
    img_binary = cv2.bitwise_or(img_binary, img_binary_3)
...
def test_run(background):
    filepath = 'F:/Temp/bn_new/0809/color_shift'
    _, file_list, _ = generate_positions(filepath)
    resultpath = 'F:/Temp/bn_new/0809/results_0to10_new'
    contrast_json = {'items': []}
    ...
    for finished_count in range(len(file_list)):
        ...
        ret, img_out, contrast_list, flake_position_list = layer_search_bn(input_name, background)
...
if __name__ == '__main__':
    ...
    bk = cv2.imread('F:/Temp/bn_new/0809/color_shift/bk.png')
    test_run(bk)
