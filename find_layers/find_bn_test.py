...
filename = 'F:/Temp/bn/small_0/2023-06-20--19-14-45-817495.jpg'
...
#%%
# threshold_lo = [int(bk_color[0] * 1.05), int(bk_color[1] * 0.9), int(bk_color[2] * 0.7)]
# threshold_hi = [int(bk_color[0] * 1.9), int(bk_color[1] * 1.6), int(bk_color[2] * 0.97)]

threshold_lo = [int(bk_color[0] * 1.05), int(bk_color[1] * 0.92), int(bk_color[2] * 0.7)]
threshold_hi = [int(bk_color[0] * 2.4), int(bk_color[1] * 2), int(bk_color[2] * 0.97)]
...
#%%
for cnt_large in cnt_large_ensemble[:3]:
    mask = np.zeros(img.shape[:2], np.uint8)
...
X = img[mask == 255]
variance_limit = np.array([6, 6, 6])
def find_segments(X, area_thresh = 200):
...
def careful_look(data, segments, area_thresh = 200):
    real_flake_list = []
    for seg in segments:
...
def calculate_contrast(sample_color, bk_color):
    ...
    '''
    contrast_range = np.array([[-0.9, -0.05], [-0.6, 0.1], [0.03, 0.3]])
...
# mask_generator = SamAutomaticMaskGenerator(sam, points_per_side = 6)
masks = mask_generator.generate(img_cnt_large_cut)
