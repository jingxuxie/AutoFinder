...
class AutoFocus():
    ...
    def focus(self, half_range, num:int, speed = 1, first_delay = 0.5, 
              delay = 0.1, fast = False, skip_thre = 20,
              **kwargs):
        
        skip_roi = [0.25, 0.75, 0.25, 0.75]
        ...
        self.img_list = []
        if not fast:
            time.sleep(0.1 + first_delay)
        self.capture()

        width = self.img.shape[1]
        height = self.img.shape[0]
        roi_w = [int(width * skip_roi[0]), int(width * skip_roi[1])]
        roi_h = [int(height * skip_roi[2]), int(height * skip_roi[3])]
        if np.max(self.img[roi_h[0]: roi_h[1], roi_w[0]: roi_w[1]]) < skip_thre: # if this is not on chip
            self.stage.move_xyz(z = - half_range, vz = speed)
    ...
    def cal_clearness(self, metric = 'default', roi = [0, 1, 0, 1], 
                      first_process = False, brightness_min = 20):
...
if __name__ == '__main__':
    temp = AutoFocus(VieworksCamera(), Stage_MCX_E1000())
    temp.initialize()
    # temp = AutoFocus(Camera(0), Stage_Rpi())
    # temp.initialize()

#%%
    _ = temp.focus(200, 10, delay = 0.18, first_delay=2.5, roi = [0.2, 0.8, 0.2, 0.8], skip_thre=1)
    plt.imshow(_[1])
    plt.colorbar()
#%%
    temp.focus(1000, 2, delay = 0.15, roi = [0.2, 0.8, 0.2, 0.8])
# %%
    temp.coarse_focus()
# %%
    temp.fine_focus()
